...
#' @param df1 the table with fish name and proce
#' 2) a table that has the number caught for each fish species for each location
#' @param df2 The table with fish name and fish caught per location
#' Data table requriements :  each location is in a different column and each fish is in a different row
#' Function Output
#' @return Port Names and Highest number of fish caught per location
#' @return rowPriceLocTot : total revenue for each location
#' @return rowPriceLocTotalAll : total fisheries revenue sum
#' @return if user requests it graph of revenue by location and total revenue (as text)
#' @export
#' End of help ..

# Import Data or read data before calling this function
...
FishPortPrice<- function(df1,df2)
{
